...
/* sector heatmaps (My List) */
.hbtn{font:600 11px/1.3 inherit;font-family:inherit;background:var(--card);border:1px solid var(--bd);color:var(--mut);border-radius:99px;padding:3px 10px;cursor:pointer;margin-left:auto;white-space:nowrap}
.hbtn:hover{color:var(--tx);border-color:var(--cyan)}.hbtn.on{color:var(--cyan);border-color:rgba(63,189,241,.5);background:rgba(63,189,241,.08)}
.heatwrap{margin:0 0 12px}
.heat{position:relative;width:100%;border-radius:10px;overflow:hidden;background:var(--bg)}
.ht{position:absolute;border-radius:4px;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;overflow:hidden;line-height:1.15;transition:filter .12s}
.ht:hover{filter:brightness(1.15);outline:2px solid var(--tx);outline-offset:-2px;z-index:2}
.ht b{font-size:13px;font-weight:800;letter-spacing:.3px}.ht span{font-size:11px;font-weight:700;font-variant-numeric:tabular-nums;opacity:.95}
.ht.sm b{font-size:10.5px}.ht.sm span{font-size:9.5px}.ht.xs b{font-size:9px}.ht.xs span{display:none}
.heatleg{display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:10.5px;color:var(--mut2);margin-top:6px}
.heatleg .hsel{display:inline-flex;border:1px solid var(--bd);border-radius:7px;overflow:hidden}
.heatleg .hsel button{font:600 10.5px/1 inherit;font-family:inherit;background:var(--panel);color:var(--mut);border:none;padding:5px 8px;cursor:pointer}
.heatleg .hsel button.on{background:rgba(63,189,241,.14);color:var(--cyan)}
.heatleg .hgrad{display:inline-block;width:120px;height:9px;border-radius:5px}
.heatleg .hcvd{display:inline-flex;align-items:center;gap:4px;cursor:pointer;user-select:none}
.heattip{position:fixed;z-index:120;pointer-events:none;background:var(--panel3);border:1px solid var(--bd2);border-radius:8px;padding:7px 10px;font-size:11.5px;color:var(--tx);box-shadow:0 6px 18px rgba(0,0,0,.35);display:none;min-width:170px;font-variant-numeric:tabular-nums}
.heattip .n{color:var(--mut);font-size:10.5px;margin-bottom:4px}.heattip div{display:flex;justify-content:space-between;gap:14px}.heattip div i{font-style:normal;color:var(--mut2)}
/* news on stock cards + detail panel */
.cnews{margin-top:9px;padding-top:7px;border-top:1px dashed var(--bd)}
.cnews .cnl{font-size:9px;font-weight:800;letter-spacing:.5px;text-transform:uppercase;color:var(--cyan);display:flex;justify-content:space-between;align-items:center}
.cnews .cnl span{font-weight:600;text-transform:none;letter-spacing:0;color:var(--mut2)}
.cni{display:grid;grid-template-columns:7px 1fr;gap:6px;align-items:start;margin-top:5px}
.cni .nwd{margin-top:5px}
.cni a{color:var(--tx);font-size:11.5px;font-weight:600;line-height:1.35;text-decoration:none;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
.cni a:hover{color:var(--cyan);text-decoration:underline}
.cni small{display:block;font-size:10px;color:var(--mut2);margin-top:1px}
.mnews .nwi{margin-bottom:7px}
.mnews{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:0 10px}
/* news rail */
...
function card(t){
 ...
 return `<div class="card" onclick="openM('${t}')">
  ...
  </div>${cardNews(t)}${researchLink(t)}</div>`;
}
...
function render(){
 ...
 if(sort==='sector'){
  board.innerHTML=DATA.sectors.map(sec=>{const ts=sec.tickers.filter(match);if(!ts.length)return'';const hid=heatHidden(sec.name);return `<div class="sector"><div class="sechd"><h2>${sec.name}</h2><span class="bl">${sec.blurb}</span><span class="cnt">${ts.length}</span>${sec.avg_change!=null?`<span class="cnt" style="color:${sec.avg_change>=0?'var(--grn)':'var(--red)'}">avg ${pct(sec.avg_change)}</span>`:''}<button class="hbtn${hid?'':' on'}" onclick="toggleHeat(${JSON.stringify(sec.name).replace(/"/g,'&quot;')})" title="${hid?'Show':'Hide'} this sector's heatmap">▦ Heatmap ${hid?'show':'hide'}</button></div>${hid?'':`<div class="heatwrap"><div class="heat" data-tk="${ts.join(',')}"></div>${heatLegend()}</div>`}${body(ts)}</div>`;}).join('')||'<div class="sub" style="padding:30px 0">No matches.</div>';
 }else{
...
window.addEventListener('resize',()=>{if(CUR)drawChart(CUR.t,CUR.tf);clearTimeout(window._hrs);window._hrs=setTimeout(drawHeats,120);});
window.addEventListener('scroll',()=>{const e=document.getElementById('heattip');if(e)e.style.display='none';},{passive:true});
...
try{const h=JSON.parse(localStorage.getItem('aiF_heat')||'null');if(h&&typeof h==='object')HEAT=Object.assign(HEAT,h,{hide:Object.assign({},h.hide||{})});}catch(e){}
function heatSave(){try{localStorage.setItem('aiF_heat',JSON.stringify(HEAT));}catch(e){}}
function heatHidden(n){return !!HEAT.hide[n];}
function toggleHeat(n){HEAT.hide[n]=!HEAT.hide[n];heatSave();render();}
function setHeat(k,v){HEAT[k]=v;heatSave();render();}
const HEATBY={chg:{l:'Day %',cap:4,v:t=>C[t].change_pct,f:v=>(v>=0?'+':'')+v.toFixed(2)+'%',lo:'−4%',hi:'+4%'},
 rsi:{l:'RSI',cap:25,v:t=>C[t].rsi==null?null:50-C[t].rsi,raw:t=>C[t].rsi,f:(v,t)=>'RSI '+Math.round(C[t].rsi),lo:'overbought 75',hi:'oversold 25'},
 up:{l:'Analyst upside',cap:50,v:t=>upside(C[t]),f:v=>(v>=0?'+':'')+Math.round(v)+'%',lo:'−50%',hi:'+50%'}};
function heatPoles(){const L=isLight();const pos=HEAT.cvd?(L?[37,99,235]:[59,130,246]):(L?[23,143,82]:[31,157,92]);const neg=L?[210,58,65]:[214,69,75];const mid=L?[226,231,239]:[42,49,66];return{pos,neg,mid};}
function heatColor(v,cap){const P=heatPoles();if(v==null||isNaN(v))return{bg:`rgb(${P.mid})`,fg:'var(--mut)'};
 const t=Math.pow(Math.min(1,Math.abs(v)/cap),0.8);const e=v>=0?P.pos:P.neg;const c=P.mid.map((m,i)=>Math.round(m+(e[i]-m)*t));
 const lum=(0.2126*c[0]+0.7152*c[1]+0.0722*c[2])/255;return{bg:`rgb(${c})`,fg:lum>0.55?'#141a26':'#fff'};}
function heatLegend(){const P=heatPoles();const g=`linear-gradient(90deg,rgb(${P.neg}),rgb(${P.mid}),rgb(${P.pos}))`;const B=HEATBY[HEAT.by];
 return `<div class="heatleg"><span>Colour</span><span class="hsel">${Object.entries(HEATBY).map(([k,o])=>`<button class="${HEAT.by===k?'on':''}" onclick="setHeat('by','${k}')">${o.l}</button>`).join('')}</span><span>${B.lo}</span><span class="hgrad" style="background:${g}"></span><span>${B.hi}</span><span>· size ∝ √ market cap · click a tile for details</span><label class="hcvd" title="Blue instead of green for positive values — easier to tell apart with red-green colour blindness"><input type="checkbox" ${HEAT.cvd?'checked':''} onchange="setHeat('cvd',this.checked?1:0)">colour-blind palette</label></div>`;}
function squarify(items,x,y,w,h){const out=[];let rest=items.slice();const total=rest.reduce((a,b)=>a+b.a,0);if(!total)return out;const scale=w*h/total;rest=rest.map(o=>Object.assign({},o,{s:o.a*scale}));
 function worst(row,side){const sum=row.reduce((a,b)=>a+b.s,0);let mx=0;for(const o of row){const r1=side*side*o.s/(sum*sum),r2=sum*sum/(side*side*o.s);mx=Math.max(mx,r1,r2);}return mx;}
 while(rest.length){const side=Math.min(w,h);let row=[rest[0]];let i=1;while(i<rest.length&&worst(row.concat(rest[i]),side)<=worst(row,side)){row.push(rest[i]);i++;}
  const sum=row.reduce((a,b)=>a+b.s,0);
  if(w>=h){const cw=sum/h;let cy=y;for(const o of row){const ch=o.s/cw;out.push(Object.assign(o,{x,y:cy,w:cw,h:ch}));cy+=ch;}x+=cw;w-=cw;}
  else{const rh=sum/w;let cx=x;for(const o of row){const cw=o.s/rh;out.push(Object.assign(o,{x:cx,y,w:cw,h:rh}));cx+=cw;}y+=rh;h-=rh;}
  rest=rest.slice(i);}
 return out;}
function drawHeats(){const B=HEATBY[HEAT.by];heatTip(null);
 document.querySelectorAll('#board .heat').forEach(el=>{const ts=(el.dataset.tk||'').split(',').filter(t=>C[t]);const W=el.clientWidth;if(!W||!ts.length){el.innerHTML='';return;}
  const H=Math.round(Math.max(110,Math.min(230,W*(ts.length>=8?0.2:ts.length>=4?0.15:0.11))));el.style.height=H+'px';
  const it=ts.map(t=>({t,a:Math.sqrt(Math.max(C[t].market_cap||0,1e9))})).sort((a,b)=>b.a-a.a);
  const G=1;el.innerHTML=squarify(it,0,0,W,H).map(o=>{const v=B.v(o.t);const col=heatColor(v,B.cap);const cls=(o.w<46||o.h<30)?'xs':(o.w<80||o.h<44)?'sm':'';
   const val=v==null?'—':(HEAT.by==='rsi'?Math.round(C[o.t].rsi):B.f(v,o.t));
   return `<div class="ht ${cls}" style="left:${(o.x+G).toFixed(1)}px;top:${(o.y+G).toFixed(1)}px;width:${Math.max(0,o.w-2*G).toFixed(1)}px;height:${Math.max(0,o.h-2*G).toFixed(1)}px;background:${col.bg};color:${col.fg}" onclick="openM('${o.t}')" onmousemove="heatTip(event,'${o.t}')" onmouseleave="heatTip(null)"><b>${o.t}</b><span>${val}</span></div>`;}).join('');});}
function heatTip(ev,t){let el=document.getElementById('heattip');if(!el){el=document.createElement('div');el.id='heattip';el.className='heattip';document.body.appendChild(el);}
 if(!ev){el.style.display='none';return;}const c=C[t];const u=upside(c);
 el.innerHTML=`<div class="n"><b style="color:var(--tx)">${t}</b> · ${c.name||''}</div><div><i>Price</i><b>${px(c.price)}</b></div><div><i>Day</i><b class="${cls(c.change_pct)}">${pct(c.change_pct)}</b></div><div><i>RSI</i><b>${c.rsi??'—'}</b></div><div><i>Analyst upside</i><b>${u==null?'—':(u>=0?'+':'')+u.toFixed(0)+'%'}</b></div><div><i>Mkt cap</i><b>${fmtB(c.market_cap)}</b></div>`;
 el.style.display='block';const x=Math.min(ev.clientX+14,innerWidth-el.offsetWidth-8),y=Math.min(ev.clientY+14,innerHeight-el.offsetHeight-8);el.style.left=x+'px';el.style.top=y+'px';}
// ---- company headlines on the stock card ("See card" + Cards view) and in the detail panel ----
function newsFor(t){const N=(DATA.news&&DATA.news.items)||[];return N.filter(i=>(i.tk||[]).some(x=>x[0]===t));}
function nwTone(i,t){const x=(i.tk||[]).find(y=>y[0]===t);return x?x[1]:i.sent;}
function cardNews(t){const L=newsFor(t);if(!L.length)return '';
 return `<div class="cnews"><div class="cnl">📰 Latest headlines<span>${L.length} in ${(DATA.news&&DATA.news.keep_h)||72}h</span></div>`+
  L.slice(0,2).map(i=>`<div class="cni"><span class="nwd ${nwTone(i,t)}" title="Tone toward ${t}: ${NWLAB[nwTone(i,t)]||'—'}"></span><div><a href="${nwEsc(i.u)}" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()">${nwEsc(i.t)}</a><small>${nwEsc(i.s)} · ${nwAgo(i.ts)}</small></div></div>`).join('')+`</div>`;}
function modalNews(t){const L=newsFor(t);const h=(DATA.news&&DATA.news.keep_h)||72;
 const hd=`<div class="sec-t">Recent Headlines — Alpha Vantage <span style="color:var(--mut2);font-weight:400;text-transform:none;letter-spacing:0">· last ${h}h · dot = tone toward ${t}</span></div>`;
 if(!L.length)return hd+`<div class="desc">No stories about ${t} made it through the news filter in the last ${h} hours.</div>`;
 return hd+`<div class="mnews">`+L.slice(0,8).map(i=>`<div class="nwi${i.k==='macro'?' macro':''}"><div class="nwm"><span class="nwd ${nwTone(i,t)}" title="Tone toward ${t}: ${NWLAB[nwTone(i,t)]||'—'}"></span><span class="src">${nwEsc(i.s)}</span><span>· ${nwAgo(i.ts)}</span></div><a class="nwt" href="${nwEsc(i.u)}" target="_blank" rel="noopener noreferrer">${nwEsc(i.t)}</a>${(i.tk||[]).length>1?`<div class="nwtk">${i.tk.filter(x=>x[0]!==t).map(x=>`<span class="nwc ${x[1]}" style="cursor:default">${x[0]}</span>`).join('')}</div>`:''}</div>`).join('')+`</div>`+
  (L.length>8?`<div class="desc" style="font-size:11px">+${L.length-8} more in the 📰 News panel — click ${t} on any story there to filter.</div>`:'');}
// ---- events calendar ----
let CALF='all';
